// Script to initialize the database schema for the Preview environment
...
// Sample data for sponsor levels
const sampleSponsorLevels = [
  { name: 'Platinum', amount: 10000.00 },
  { name: 'Gold', amount: 5000.00 },
  { name: 'Silver', amount: 2500.00 },
  { name: 'Bronze', amount: 1000.00 },
  { name: 'Friend', amount: 500.00 }
];
...
async function initializeDatabase() {
  ...
  try {
    ...
    // Check if sponsor_levels table has data
    const hasLevelsData = await tableHasData(pool, 'api', 'sponsor_levels');
    
    // Insert sample data if the table is empty
    if (!hasLevelsData) {
      console.log('ℹ️ Sponsor levels table is empty, inserting sample data...');
      
      for (const level of sampleSponsorLevels) {
        await pool.query(`
      ...
      console.log('✅ Sample sponsor levels inserted');
    } else {
      console.log('ℹ️ Sponsor levels table already has data, skipping sample data insertion');
    }
